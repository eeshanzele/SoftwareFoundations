...
Lemma t_update_eq : forall (A : Type) (m : total_map A) x v,
  (x !-> v ; m) x = v.
Proof.
  intros A m x v. unfold t_update. rewrite String.eqb_refl.
  reflexivity.
Qed.
...
Theorem t_update_neq : forall (A : Type) (m : total_map A) x1 x2 v,
  ...
  (x1 !-> v ; m) x2 = m x2.
Proof.
  intros A m x1 x2 v H. unfold t_update. unfold not in H.  
   apply String.eqb_neq in H. destruct String.eqb.
    - inversion H.
    - reflexivity. 
Qed. 
...
Proof.
  ...
  unfold t_update. destruct String.eqb. 
    - reflexivity. 
    - reflexivity. 
Qed.
...
Proof.
   ...
   intros x'. unfold t_update. destruct (String.eqb_spec x x').
...
Theorem t_update_permute : forall (A : Type) (m : total_map A)
  ...
  (x2 !-> v2 ; x1 !-> v1 ; m).
Proof.
  intros. apply functional_extensionality. unfold t_update. 
  intros x. destruct (String.eqb_spec x1 x). 
    - rewrite <- String.eqb_neq in H. rewrite <- e. 
