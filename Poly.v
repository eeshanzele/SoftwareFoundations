...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
intros X l m n. induction l as [| h' t' IHl']. 
  - reflexivity. 
  - simpl. rewrite <- IHl'. reflexivity. 
Qed. 
...
Definition filter_even_gt7 (l : list nat) : list nat := 
  filter (fun x => andb (even x) (gt7 x)) l.
...
Theorem map_append : forall (X Y : Type) (f : X -> Y) (l1 : list X) (l2 : list X),
  map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof.
  intros X Y f l1 l2. induction l1 as [|h' t' IHl1'].
  - reflexivity. 
...
Proof.
  ...
  - reflexivity. 
  - simpl. rewrite -> map_append. rewrite -> IHl'. reflexivity. 
Qed.
...
Theorem correct_map: forall X Y (l : list X) (f: X -> Y), 
  fold_map f l = map f l.

Proof.
  intros X Y l f. induction l as [| h' t' IHl'].
  - reflexivity.
  - simpl. rewrite <- IHl'. reflexivity. 
Qed.  
