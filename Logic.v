...
Proof.
  intros n m eq1. destruct n as [|n'].
  - simpl in eq1. split. reflexivity. apply eq1. 
  - discriminate eq1. 
Qed. 
...
Proof.
  intros n m eq1. destruct n. 
  - left. reflexivity. 
  - right. simpl in eq1. destruct m as [|m'].
    * reflexivity.
    * discriminate eq1.
Qed.
...
Theorem or_commut : forall P Q : Prop,
  P \/ Q  -> Q \/ P.
Proof.
  intros P Q eq1. destruct eq1 as [HP | HQ].
  - right. apply HP. 
  - left. apply HQ.
Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
 intros P Q A. unfold not. intros B C. apply B in A.
   - apply A. 
   - apply C.
Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros H. destruct H as [A B].
  apply B in A. apply A. 
Qed. 
...
Theorem de_morgan_not_or : forall (P Q : Prop),
    ~ (P \/ Q) -> ~P /\ ~Q.
Proof.
  intros P Q. unfold not. intros H. split. 
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R A B. split.
  - intros C. apply A in C. apply B in C. apply C.
  - intros D. apply B in D. apply A in D.  apply D.
Qed.
...
Proof.
  ...
  - intros A. split. 
    ...
    +  destruct A. 
      * left. apply H.
      * right. apply H. 
  - intros B. destruct B as [A B]. destruct A as [A1 | A2].
    + left. apply A1. 
    + destruct B as [B1 | B2].
      * left. apply B1.
      * right. split. apply A2. apply B2. 
Qed. 
...
Proof.
  intros X P A B. unfold not in B. destruct B as [x E]. apply E. 
  apply A. 
Qed.
...
Proof.
  intros X P Q. split. 
    - intros A. destruct A. destruct H.  
      +  left. exists x. apply H. 
      +  right. exists x. apply H. 
    - intros B. destruct B. destruct H.
...
Proof.
  intros A B f l y. split. induction l as [| h' t' IHl'].
  ...
  - simpl. intros H. destruct H. 
    ...
    + destruct IHl'. 
      * apply H. 
      * exists x. split. apply H0. destruct H0. right. apply H1.
  - intros H. destruct H. destruct H. rewrite <- H. apply In_map. 
    apply H0. 
Qed. 
...
Proof.
 intros A l. split. 
    - induction l as [|h t IHl].
        ...
        * simpl. intros [H|H].
           + left. left. apply H. 
           + apply IHl in H. destruct H. left. right. apply H. 
             right. apply H. 
    - induction l as [|h t IHl]. 
        * simpl. intros [H | H].
           +  destruct H.
...
Proof.
  intros n. induction n as [|n' IHn'].
    ...
    - destruct IHn' as [k IHn']. rewrite even_S. destruct (even n').
